...
//}
Test (sfmm_student_suite, check_zero_peak_utilization_and_internal_fragmentation, .timeout = TEST_TIMEOUT){
	double peak_utilization = sf_peak_utilization();
	double internal_fragmentation = sf_internal_fragmentation();
	cr_assert_eq(internal_fragmentation, 0.0, "Not the correct internal fragmentation. Was expected 0 (or 0.0), but got %.1f", internal_fragmentation);
	cr_assert_eq(peak_utilization, 0.0, "Not the correct peak utilization. Was expected 0 (or 0.0), but got %.1f", peak_utilization);
}

Test (sfmm_student_suite, sfpu_sfiu_test_2, .timeout = TEST_TIMEOUT){
	sf_malloc(3000);
	sf_malloc(357);
	sf_malloc(3627);

	double sfpu = sf_peak_utilization();
	double sfiu = sf_internal_fragmentation();

	cr_assert_float_eq(sfiu, 0.992045, 0.001, "Not the correct internal fragmentation. Expected was: 0.992045, but got %.6f", sfiu);
	cr_assert_float_eq(sfpu, 0.97433, 0.0001, "Not the correct peak utilization. Expected was 0.97433, but got %.5f", sfiu);
}

Test (sfmm_student_suite, realloc_test, .timeout = TEST_TIMEOUT){
	sf_errno = 0;

	void *ptr = sf_malloc (310);
	void *ptr2 = sf_malloc (6929);
	
	ptr = sf_realloc(ptr, 792);
	ptr2 = sf_realloc(ptr, 4963);

	ptr = sf_realloc(ptr2, 555);

	assert_free_block_count(0,3);
	cr_assert(sf_errno == 0, "sf_errno should be 0");
}

Test (sfmm_student_suite, free_test, .timeout = TEST_TIMEOUT){
	sf_errno = 0;

	int count = 0;
	while (count != 10){
		void* ptr = sf_malloc(708);
        sf_free(ptr);
        count++;
    }
...
Test (sfmm_student_suite, malloc_test, .timeout = TEST_TIMEOUT){
	sf_errno = 0;

	int start = 200;
    void *ptr = sf_malloc(start);
    while (ptr != NULL){
        start = start * 2;
        ptr = sf_malloc(start);
    }
	...
	assert_free_block_count(0, 1);
	cr_assert(sf_errno == ENOMEM, "sf_errno should be 12");
}